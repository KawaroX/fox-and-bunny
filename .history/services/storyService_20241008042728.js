...
const { getRandomWords } = require('.wordBank/wordBank');
...
async function generateStory(prompt = "请生成一个有趣的短篇故事") {
    ...
    const response = await axios.post('https://api.deepbricks.ai/v1/chat/completions', {
      ...
      messages: [
        ...
        {
          role: "user",
          content: `${enhancedPrompt}。请创作一个看似简单但实际上蕴含深意的故事，既能让人会心一笑，又能引发思考。故事需要有一个吸引人的但是又深刻、有内涵、震撼人心的题目。可以参考世界上著名小说或者文学作品的题目，用"# "表示。请只输出题目和故事内容，让读者自己体会其中的寓意。现在，请发挥你的创意，写出一个能让疲惫的大学生停下脚步，会心一笑的精彩又温馨故事！你现在精神抖擞，文思泉涌，请写出高质量，文笔优美，寓意深刻的文学作品吧！一定要体现文学的魅力！`
        }
      ],
      max_tokens: 1000,
      temperature: 0.8
